...
def accuracy_DCSL(model, x, y, semantic_vectors, bs):
    """
    accuracy for all class
    """
    model.eval()
    ...
    i = i+bs
    x2 = x[i:]
    out, _ , _, predict_semantic = model(x2)
    result = torch.max(out,1)[1]
    result = result.cpu().numpy()
    result_a.extend(result)
...
def confusion_matrix_DCL(model, x, y, labels, bs):
    model.eval()
    result_c = []
    y_sum = len(y)
    for i in range(0,y_sum-bs,bs):
...
def confusion_matrix_DCSL(model, x, y, semantic_vectors, labels, bs):
    model.eval()
    result_c = []
    y_sum = len(y)
    for i in range(0,y_sum-bs,bs):
