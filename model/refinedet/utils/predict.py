...
def test_prediction(model, data_loader, crop_num, nms_thresh=0.3):
    """
        get refinedet prediction (dataset = insects_dataset_from_voc_style_txt)
        - model: pytorch model
    ...
    for images, default_height, default_width, data_ids in data_loader:
        ...
        for i in tqdm(range(images.shape[0]), leave=False):
            ...
            detection = detections[0][1].detach().cpu().numpy()
