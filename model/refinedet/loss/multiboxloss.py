...
class RefineDetMultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        num_priors = loc_data.size(1)  # need check
        prior_data = prior_data[:num_priors, :]  # need check
        ...
        for idx in range(batch_size):
            ...
            if self.use_ARM is True:
                ...
                target_labels = target_labels >= 0  # can't understand ...
                refine_match(self.overlap_thresh, target_boxes, prior_data, self.variance, target_labels,
                             loc_t, conf_t, idx)
        if self.use_gpu:
            loc_t = loc_t.cuda()
        ...
        if self.use_ARM is True:
            arm_conf_probability = F.softmax(arm_conf_data, 2)
            arm_conf_object_probability = arm_conf_probability[:, :, 1]
            object_score_index = arm_conf_object_probability <= self.theta
            object_filter = conf_t > 0
            object_filter[object_score_index.detach()] = 0
        else:
            object_filter = conf_t > 0

        # Localization Loss (Smooth L1)
        # object_filter.shape == [batch_size, num_priors]
        # object_filter_for_loc_data.shape == [batch_size, num_priors, 4]
        object_filter_for_loc_data = object_filter.unsqueeze(object_filter.dim()).expand_as(loc_data)
        loc_p = loc_data[object_filter_for_loc_data].view(-1, 4)
        loc_t = loc_t[object_filter_for_loc_data].view(-1, 4)
        loss_l = F.smooth_l1_loss(loc_p, loc_t)

        # Compute max conf across batch for hard negative mining
        # batch_conf.shape == [batch_size*num_priors, self.num_classes]
        ...
        neg, sum_pos = hard_negative_mining(loss_c, object_filter, self.negpos_ratio)

        # Confidence Loss Including Positive and Negative Examples
        object_filter_for_loc_data = object_filter.unsqueeze(2).expand_as(conf_data)
        neg_idx = neg.unsqueeze(2).expand_as(conf_data)
        conf_p = conf_data[(object_filter_for_loc_data + neg_idx).gt(0)].view(-1, self.num_classes)
        targets_weighted = conf_t[(object_filter + neg).gt(0)]
        if self.use_CSL is True:
            loss_c = F.cross_entropy(conf_p, targets_weighted, weight=torch.tensor(self.CSL_weight, dtype=torch.float32).cuda())
        else:
            loss_c = F.cross_entropy(conf_p, targets_weighted)

        # Sum of losses: L(x,c,l,g) = (Lconf(x, c) + αLloc(x,l,g)) / sum_pos
        loss_l /= sum_pos
        loss_c /= sum_pos
