...
class Detect(Function):
    ...
    def forward(ctx, arm_loc_data, arm_conf_data, odm_loc_data, odm_conf_data, prior_data, \
               num_classes, background_label=0, top_k=1000, keep_top_k=500, nms_thresh=0.45, \
               conf_thresh=0.01, objectness_thresh=0.01, variance=[0.1, 0.2]):
        # get confidence from source conf_data
        arm_conf_data = F.softmax(arm_conf_data, dim=-1)
        odm_conf_data = F.softmax(odm_conf_data, dim=-1)
