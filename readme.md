...
- 2020/8  
    ...
    - データ追加後の検出結果が悪化した理由を調べる  
        - 全体的に昆虫が大きくなって、緩衝地帯が足りなくなった?  
        →画像の1辺を1024pixelにして入力。変わらない。  
        - 新しい画像では光の当たり方が違う  
        →ヒストグラム平滑化。結果悪くなった。  
        - その他の昆虫が大量に存在する(small、mediumから移った)  
        →データセット再構築。結果悪くなった。  
        - 学習率を下げ、バッチサイズを増やした  
        →結果悪くなった。  
        - 新旧の画像で昆虫の大きさが異なる可能性がある  
        →ランダムなリサイズやクロップが必要。  
        - ラベル付けは問題なし(多少雑だったりするが...)  
    - 分類データセットを見て、昆虫がはみ出していないか確かめる  
        - はみ出してる昆虫もいるが、分類出来そうなので保留  
    - 検出にimgaugを用いたデータ拡張を実装  
        - これにより、SPREAD_ALL_OVERの関数が使用できなくなった  
        →学習時とテスト時で処理される画像が異なるバグがある  
